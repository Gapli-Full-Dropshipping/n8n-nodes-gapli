			...
			try {
				...
				// ─── Order ──────────────────────────────────────────────
					...
					if (operation === 'get') {
						...
						// Bloki `conversation` i `diagnostics` są RODZEŃSTWEM `order`
						// w odpowiedzi, a nie jego polami. Zwrócenie samego `response.order`
						// po cichu by je gubiło — dlatego doklejamy je do wyniku.
