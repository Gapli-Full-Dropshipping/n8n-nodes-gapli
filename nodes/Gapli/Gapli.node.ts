				...
				if (resource === 'product') {
					...
					if (filters.exclude_parser_ids) qs.exclude_parser_ids = filters.exclude_parser_ids;
						...
						if (scanAll) {
							...
							// Step 2: Iterate each parser page-by-page
								...
								while (hasMore) {
									...
									if (hasMore) {
										await new Promise<void>((resolve) => {
											const t = globalThis.setTimeout(resolve, INTER_PAGE_DELAY_MS);
							...
							// Attach summary meta to each product for downstream auditing
							if (summaryMeta) {
								for (const product of allProducts) {
									product._meta = summaryMeta;
								}
							}
