		...
		options: [
			{
				...
				value: 'checkOffer',
				description:
					'Check whether an offer already sits in a bundle. Allegro presents only ONE bundle per offer, so a Create touching an occupied offer is always refused — and bundles Allegro generates on its own count too.',
				action: 'Check whether an offer is already bundled',
			},
			{
				name: 'Create',
				value: 'create',
...
export const bundleFields: INodeProperties[] = [
	...
	},
	{
		...
		description:
			'Whether to follow the cursor until every bundle is returned. Allegro caps one page at 1000 bundles, and an account that sells across many offers routinely holds far more — an account with 13,000 bundles needs 14 pages. Requires an Allegro account to be selected, because a cursor is only valid within one account.',
	},
	{
		displayName: 'Limit',
		name: 'limit',
		...
		displayOptions: {
			show: {
				resource: ['bundle'],
				operation: ['getAll'],
				bundleReturnAll: [false],
			},
		},
		// Opis jest narzucony przez lintera n8n co do znaku — sufit 1000 i sposób
		// dojścia do reszty opisuje przełącznik Return All wyżej.
	...
	// ── Check Offer ─────────────────────────────────────────────
	{
		...
		description:
			'Allegro offer to test. Returns bundled=true/false plus the bundle holding it. This finds the offer even when the listing shows a different one for that slot, so it is the only reliable way to tell a free offer from an occupied one.',
	},
	...
	},
	{
		...
		displayOptions: {
			show: {
				resource: ['bundle'],
				operation: ['get', 'create', 'update', 'delete', 'checkOffer'],
			},
		},
		description:
