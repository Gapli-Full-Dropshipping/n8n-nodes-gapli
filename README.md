...
| **Get by ID** | Retrieve a single order by its numeric ID. Optional `Include`: **Conversation** (buyer messages, disputes, claims) and **Diagnostics** (wholesaler per item, status timings, issue reason codes, warning flags). Both ride along on the same request — no extra HTTP calls. |
