...
> 🚚 **`shipments[]` comes out of Get by ID and nowhere else.** `Get Many` does not contain it, so
> a workflow polling the list finds nothing and concludes there is no tracking. Every "tracking
> number back to my own shop" flow runs through **Get by ID**.
>
> ⚠️ **Skip entries whose `direction` is `inbound_from_wholesaler`.** That leg is the transport
> wholesaler → GAPLI warehouse, **not** the parcel to the buyer, and must never be shown to the
> customer. The buyer's parcel carries `direction: "outgoing"`.
...
> **This node never talks to Shopify or WooCommerce.** It talks to GAPLI, and GAPLI does the work
> on the store side — over the Shopify Admin API or the WooCommerce plugin. You say "stock my store
> with these SKUs" and **no access to your own store's API is involved** at any point. One call looks
> the same whichever engine the store runs.

| Operation | Description |
|---|---|
| **Get Many** | Retrieve all stores (Shopify, WooCommerce, PrestaShop, etc.) for the authenticated operator. |
| **Send Products** | Stock the store from the GAPLI catalogue. Optional pricing (percentage markup, flat markup, profit reduction) and store category IDs. Scope `store:products:write`. Max **100 SKUs** per request. |
| **Get Assortment** | What actually stands in the store, SKU by SKU, with `external_id`, `first_seen_at`, `last_synced_at`, `never_synced` and `last_error`. Scope `store:products:read`. |
| **Get Statistics** | The store's **own** sales for a period: orders by status, revenue from paid orders, average order value, assortment size. Scope `store:statistics:read`. |

> ⏳ **Send Products waits, and it has a 100-SKU ceiling.** Shopify accepts one product per
> points-metered mutation (~30 cards/min per store), so a 100-item request runs for **several
> minutes**. The node allows 10 minutes for it and deliberately does **not** retry a timeout.
> **Never retry it yourself either** — a broken wait tells you nothing about how much already
> landed, and a second call stocks the store twice. Read **Get Assortment** instead.

> ⚠️ **On WooCommerce `sent_count` means HOW MANY WERE SENT, not how many the store accepted.**
> The plugin takes the batch asynchronously, so the response carries a warning saying exactly that.
> The node returns the whole response — `warnings[]` included — so **read it**; confirmation only
> ever comes from **Get Assortment**.

> ⚠️ **The store product limit applies here, exactly as it does in the panel.** Over the limit the
> batch is **TRIMMED**, and only `trimmed_by_limit` tells you by how much — `sent_count` on its own
> looks like a full success. `limit` reports the state (`limit`, `currentCount`, `available`). When
> the limit blocks the batch entirely the request fails with HTTP 422 and the reason is now in the
> error message rather than a bare "request failed".

> 💡 **In Get Assortment, read `_meta.source` before concluding a product is missing.**
> `shopify_product_map` is GAPLI's own dispatch register — **accurate to the minute**.
> `store_snapshot` is a sweep filled in round-robin that reaches some stores only **once every few
> days**: check `_meta.snapshot_updated_at` first. `never_synced: true` means the card **is** in the
> store but has never received a stock sync — the first place to look at "the product shows the
> wrong stock". `_meta` is attached to every returned item, and a store holding nothing still
> returns one item carrying `store`, `total` and `_meta`, so a stale sweep never arrives as silence.

> ⚠️ **Get Statistics is the store's own sales, not marketplace sales.** Allegro and Erli orders
> also carry a `store_id` — the store their marketplace account is pinned to — so a counter built on
> `store_id` alone returns a figure several times too high and entirely believable. Average order
> value is computed from **paid** orders only, so a cart abandoned in status "new" does not drag it
> down.
