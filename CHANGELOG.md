...
- **Store — stock your own shop from the GAPLI catalogue, read back what stands in it, and count what it sold.** The resource could previously only list stores, so the one thing an operator actually wants from their own shop — putting products in it — had no route through n8n at all. Three operations now do: **Send Products**, **Get Assortment**, **Get Statistics**.
  - **The node does not talk to Shopify or WooCommerce.** It talks to GAPLI, and GAPLI does the work on the store side — over the Shopify Admin API or the WooCommerce plugin. You say "stock my store with these SKUs" and **no access to your own store's API is involved**, so one flow looks identical whichever engine the store runs. Only stores with a real sales engine qualify; a "virtual" store is a bridge to a marketplace account and is refused with 409.

- **Send Products waits, and that is the whole design of it.** Shopify accepts one product per points-metered mutation — about **30 cards a minute per store** — so the 100-SKU ceiling means a full batch runs for **several minutes**. The node allows **10 minutes** for the request instead of letting it die on a default timeout, and it deliberately does **not** retry one. A broken wait tells you nothing about how much already landed, and a second call stocks the store twice; **Get Assortment** is the answer to "what got through", not a repeat send.
  - **`sent_count` is not a confirmation on WooCommerce.** The plugin takes the batch asynchronously, so the number means HOW MANY WERE SENT, not how many the store accepted. The response says so in `warnings[]`, and the node returns the **whole** response rather than a tidy pair of counters — losing that array would turn a warning into a silent assumption.
  - **`trimmed_by_limit` is the field that stops a flow from lying to you.** The store product limit is enforced on this endpoint exactly as it is behind the panel button, and over the limit the batch is **TRIMMED** rather than refused. `sent_count` then reports a perfectly successful send of a batch that was quietly cut — only `trimmed_by_limit` says by how much, with `limit` carrying the state (`limit`, `currentCount`, `available`).

- **Get Assortment: `_meta.source` decides what the list is worth.** `shopify_product_map` is GAPLI's own dispatch register, accurate to the minute. `store_snapshot` is a sweep filled in round-robin that reaches some stores **once every few days** — so on that source a missing SKU means "the sweep has not been here since `_meta.snapshot_updated_at`", not "the product is not in the store". `never_synced: true` is the third state: the card **is** in the store but has never received a stock sync, which is the first place to look at "the product shows the wrong stock".
  - `_meta` rides on every item, because it is a sibling of the list in the API and returning only the products would drop the one field that tells you how much to trust them. A store holding nothing still returns one item carrying `store`, `total` and `_meta` — an empty array would make a stale sweep indistinguishable from an empty store.
  - **Return All follows `total`, not `has_more`.** This endpoint caps a page at 500 and returns `total` with **no `pagination` block**, so the package's shared paginator — which stops exactly when that block is absent — would have handed back the first 500 items of a 1200-product store as if that were the whole store.

- **Get Statistics is the store's own sales, and only that.** Allegro and Erli orders also carry a `store_id` — the store their marketplace account is pinned to — so a "store revenue" counter built on `store_id` alone returns a figure several times too high and completely believable. The endpoint filters on the platform as well. Average order value comes from **paid** orders, not from all of them, so a cart abandoned in status "new" does not drag it down.

### Fixed

- **A batch blocked by the store product limit no longer arrives as `GAPLI API request failed`.** That refusal comes back as HTTP 422 with its explanation in `message` — "limit 1000, in store 1000, available 0" — and the node only ever read `error`, so the one endpoint whose failure needs the numbers was the one that discarded them. `message` is now read as a fallback, and any `warnings[]` sent alongside an error go into the message too, since nobody reads a response body after an exception.

### Changed

- **Order → Get by ID now says where the tracking number comes from.** The data was always there — `shipments[]` has been passed through untouched — but nothing in the node mentioned that `Get Many` does **not** contain it, so a flow polling the list finds nothing and concludes there is no tracking. It also now warns about `direction: "inbound_from_wholesaler"`: that leg is the transport wholesaler → GAPLI warehouse, not the parcel to the buyer, and showing it to a customer hands them a tracking number for a shipment to our own warehouse. The buyer's parcel is `direction: "outgoing"`.

## [1.8.0] - 2026-09-08

### Added

- **Bundle → Check Offer.** Allegro presents **one bundle per offer**, and an offer already taken refuses every Create that touches it. This asks the question directly: pass an offer ID, get `bundled` true/false plus the bundle holding it and its `created_by`. It is one request instead of a rejected write, and — unlike the listing — it finds the offer even when it sits deeper in a slot.
  - The occupancy cannot be derived from Get Many. Measured on a live account holding 13,714 bundles: a bundle averages 3.3 slot representatives against 26.4 offers, so the listing shows roughly **one occupied offer in eight**. A workflow that builds its "already taken" set from `slots_representatives` will keep proposing pairs Allegro then refuses.
  - `created_by` matters as much as `bundled`. On that same account **13,655 of 13,714 bundles were created by Allegro itself**, not by the seller — and an Allegro-generated bundle occupies the offer exactly like your own. Deleting "all my bundles" does not free them.

- **Bundle → Get Many: Return All.** Allegro caps a page at 1000 bundles, so an account with more than that was silently truncated at the first page — there was no way to reach the rest, because the node dropped the cursor before the workflow could see it. Return All follows the cursor to the end (that 13,714-bundle account takes 14 pages). It needs an account selected, since a cursor is only valid within one.

### Fixed

- **A refusal from Allegro no longer arrives as "Bad gateway".** Every Allegro error except 404 and 429 used to leave the API as HTTP 502, which reads as "the service broke, try again" — so workflows retried a permanent validation refusal, three times, forever. Allegro's 4xx now passes through as a 4xx (401/403 become 424, so they are never mistaken for a problem with the GAPLI credential), and the response carries **`retryable`** to decide on rather than a status code to guess from.
- **The node now shows the reason a request failed.** It used to report `GAPLI API request failed` and let n8n fill in its stock line for the status code, discarding the explanation the API had already sent. The message is now the API's own, with `Allegro HTTP <status> · <code> · permanent, do not retry` alongside it.
