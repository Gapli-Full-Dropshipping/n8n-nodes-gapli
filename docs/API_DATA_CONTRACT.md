...
5. [Store Assortment — Stocking, Reading, Counting](#5-store-assortment)
...
**This is a layer, not a proxy.** The node talks to GAPLI; GAPLI talks to the store — over the
Shopify Admin API or the WooCommerce plugin. The operator needs **no access to their own store's
API**, and one call looks the same whichever engine the store runs.

| Endpoint | Scope | Node operation |
|---|---|---|
| `POST /store/products` | `store:products:write` | Store → Send Products |
| `GET /store/products` | `store:products:read` | Store → Get Assortment |
| `GET /store/statistics` | `store:statistics:read` | Store → Get Statistics |

Only stores with a **real sales engine** carry an assortment. A "virtual" store is a bridge to a
marketplace account — no assortment to stock, no own sales to count — and is refused with **HTTP
409** (`store_has_no_engine`).

### `POST /store/products` — stocking the store

Max **100 SKUs** per request. The ceiling is the store's throughput, not a database limit.

```json
{
  "store_id": 123,
  "product_skus": ["13470_136", "13471_136"],
  "pricing": { "markup_increase_percent": 15 },
  "category_ids": [12, 34]
}
```

`pricing` is applied in this order, and the order matters: percentage markup on the catalogue price,
then the flat markup, then `profit_reduction_percent` — which cuts **only what the two markups
added** and can never take the price below the catalogue price.

**The request blocks.** Shopify accepts one product per points-metered mutation (~30 cards/min per
store), so 100 items run for several minutes. The node allows **10 minutes** and does **not** retry a
timeout, by design: a broken wait says nothing about how much already landed, and a second call
stocks the store twice. Confirm with `GET /store/products` instead of retrying.

| Response field | What it actually means |
|---|---|
| `sent_count` | Shopify: cards created. **WooCommerce: HOW MANY WERE SENT**, not how many the store accepted — the plugin takes the batch asynchronously. |
| `skipped_count` | Items the handler did not send (already present, ineligible). |
| `error_count` | Items that failed on the store side. |
| `trimmed_by_limit` | **How many items the store's product limit cut off.** `0` means the limit did not bite. Non-zero means part of the batch was never sent, while `sent_count` still looks like a success. |
| `limit` | `{ limit, currentCount, available }` — the state of the store's product limit. |
| `warnings[]` | Things that went through but **change the meaning of the result**. Read them. |

The store product limit is enforced here, not only in the panel — a public endpoint that skipped it
would be the simplest way around a paid limit. Over the limit the batch is **trimmed**; when nothing
fits at all the request returns **HTTP 422** and the reason arrives in the error message.

### `GET /store/products` — what actually stands in the store

`_meta.source` is the field that decides how much the list is worth:

| `_meta.source` | What it is | How fresh |
|---|---|---|
| `shopify_product_map` | GAPLI's own dispatch register | **Accurate to the minute** |
| `store_snapshot` | A sweep filled in round-robin | Reaches some stores **once every few days** — check `_meta.snapshot_updated_at` before concluding anything |

`never_synced: true` means the card **is** in the store but has never received a stock sync — the
first place to look at "the product shows the wrong stock". `external_id` is `null` on WooCommerce:
the plugin addresses everything by SKU and never returns a card identifier.

One page holds at most **500** items and the response carries `total` but **no `pagination` block**,
so a paginator waiting for `has_more` stops after the first page. The node's **Return All** walks
`total` instead. `_meta` is attached to every item it returns, and an empty store still yields one
item carrying `store`, `total` and `_meta` — otherwise a stale sweep would be indistinguishable from
an empty store.

### `GET /store/statistics` — the store's own sales

**Own sales only, never marketplace.** Allegro and Erli orders also carry a `store_id` — the store
their marketplace account is pinned to — so the query filters on `store_id` **and** on the platform
matching the engine slug. Counted on `store_id` alone the figure comes out several times too high
and entirely believable.

`revenue.average_order_value` is computed from **paid** orders (`orders.paid`), not from
`orders.total`, so a cart abandoned in status "new" does not drag it down. `revenue.gross` likewise
sums paid orders only.
